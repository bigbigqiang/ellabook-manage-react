...
import { Row, Col, Card, Icon, Button, Table, Input, Upload, Modal, Checkbox, DatePicker, notification, message, Tooltip, Select, Spin } from "antd";
...
export default class addGoods extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      bookSelectList: [],     //这是选择的结果列表(全套数据)
      bookSelectCodeList: [],  //这里是选择的bookCode列表
      bookSelectNameList: [],    //这里是返回的展示列表bookName
      goodsType: "",
      targetTypeList: [],
      dailyDesc: '',
      loading: false,
  ...
  async fetchBookList() {
    ...
    console.log("qqqqqq")
    console.log(this.state.bookSelectCodeList)
    var doc = {
      text: '',
      pageSize: 2000,
      type: "SEARCH_ALL"
    ...
    //初始值设置
    console.log(this.state.bookSelectCodeList)
    this.state.bookSelectCodeList.forEach((item) => {
      var result = data.data.bookList.find(c => c.bookCode == item)
    ...
    this.setState({
      ...
      bookSelectList: bookSelectList,
      loading: false
    }, () => { console.log(this.state.bookSelectList); this.goodsTypeChange(this.state.goodsType); }
    )
    // console.log(data);
  }
  // TODO:搜索图书包
  async fetchBookPack() {
    ...
    console.log(this.state.bookSelectCodeList);
    if (!data.data) return;
    var bookSelectList = this.state.bookSelectList;
    this.state.bookSelectCodeList.forEach((item) => {
      var result = data.data.list.find(c => c.thirdCode == item)
      bookSelectList.push(result);
      console.log(result);
    })
  ...
  //////////////////////保存模态框////////////////////////
  async fetchDefaultData(dailyCode) {
    var doc = {
      dailyCode
    }
    var data = await fetch(getUrl.url, {
      ...
      mode: 'cors',
      body: "method=ella.operation.getDailyInfo" + "&content=" + JSON.stringify(doc) + commonData.dataString
    }).then(function (res) {
      return res.json();
    });
    console.log(data);
    ...
    this.setState({
      dailyCode: data.data.dailyCode,
      dailyTitle: data.data.dailyTitle,
      dailyContent: data.data.dailyContent,
      dailyImg: data.data.dailyImg,
      effectDate: data.data.effectDate,
      targetPage: data.data.targetPage,
      authorName: data.data.authorName,
      dailyContent2: data.data.dailyContent, //暂存数据
      goodsType: data.data.goodsType,
      dailyDesc: data.data.dailyDesc,
      fileList: data.data.dailyImg ? [
        ...this.state.fileList,
        {
          uid: -1,
          name: 'xxx.png',
          status: 'done',
          url: data.data.dailyImg,
        }
      ] : [],
      //获取储存的bookCode,如果之前没有储存就返回[];
      bookSelectCodeList: data.data.bookCode ? data.data.bookCode.split(",") : []
  ...
  storeFetch = async (publishFlag) => {
    ...
    console.log(targetPage.data)//TODO:这里打印的是网址
    ...
    var doc = {
      dailyCode: this.state.dailyCode,
  ...
  searchRedContent(value) {
    this.getBookBagSelectList(value);
  ...
  render() {
    ...
    const effectDate = this.state.effectDate || "2017-11-11 11:11:11"
    console.log("123321123321")
    // console.log(this.state.bookSelectCodeList.join(','))
    return (
      <div className="add_daily">
        <h2 style={title}><Link style={font_color} to="/remittance" ><Icon style={back} type="left" />编辑每日绘本</Link></h2>
            ...
            <Row>
              ...
              <Col span={16}>
                <Row style={{ ...marginBottom }}>
                  ...
                  <Col span={6}>
                    {
                      <Input value={this.state.dailyTitle ? this.state.dailyTitle : ""} onChange={(e) => { this.getContent("dailyTitle", e.target.value) }} placeholder="请输入标题" />
                    }
                  </Col>
                ...
                <Row>
                  ...
                  <Col span={6}><Input value={this.state.authorName ? this.state.authorName : ""} onChange={(e) => { this.getContent("authorName", e.target.value) }} placeholder="请输入作者" /></Col>
                </Row>
                ...
                <Row >
                  <Col style={{ margin: "20px 0px 0px 80px" }} span={20} className="gutter-row" >
                    {/* <Editor style={{ width: "600px" }} titleImg={this.state.isInsert ? this.state.titleImg : null} getContent={this.getContent.bind(this)} dailyContent={this.props.params.content}></Editor> */}
                    <Editor style={{ width: "600px" }} titleImg={this.state.isInsert ? this.state.titleImg : null} getContent={this.getContent.bind(this)} dailyContent={this.state.dailyContent2}></Editor>
                  </Col>
                </Row>
                <Row style={{ ...marginBottom }}>
                  ...
                  <Col span={2} style={{ "min-width": "120px" }}>
                    ...
                    <Select labelInValue showArrow value={{ key: this.state.goodsType }} style={{ width: '100%' }} onChange={(v) => { this.handleSelectChange(v.key) }}>
                      {
                ...
                <Row style={{ ...release }}>
                  <Col style={{ ...lineHeight_30 }} span={1}>
                    {/*<Checkbox onChange={this.isSetReleaseTime}>定时发布props.params.content</Checkbox>*/}
                    定时发布
                </Col>
                  <Col span={6}>
                    <DatePicker
                ...
                <Row style={{ marginTop: "50px" }}>
                  <Col style={{ ...lineHeight_30 }} span={3} offset={4}>
