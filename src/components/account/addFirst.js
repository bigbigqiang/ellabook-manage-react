...
class firstPartner extends React.Component {
    ...
    async addFn(str) {
        ...
        var data = await fetch(getUrl.url, {
            ...
            body: "method=ella.business.addBusiness" + "&content=" + JSON.stringify(str) + commonData.dataString
        }).then(res => res.json())
            .then((d) => {
    ...
    save() {
        if (this.state.list.businessTrueName == '' || this.state.list.businessTrueName == null) {
            message.warning('名称不能为空')
        } else if (this.state.list.businessName == '' || this.state.list.businessName == null) {
            message.warning('联系人不能为空')
        } else if (this.state.list.mobile == '' || this.state.list.mobile == null) {
        ...
        // else if (this.state.list.divideType == 'PERCENT_DIV' && (this.state.list.dividePercent == '' || this.state.list.dividePercent == null)) {
        //     message.warning('分成比例不能为空')
        // }  
        else if (this.state.list.contractStartTime>this.state.list.contractEndTime) {
            message.warning('开始时间不能大于结束时间')
        } else {
            this.addFn(this.state.list);
        }
    }
    //下拉接口
    async selectFn() {
                ...
                <div className="m-firstDetail-bd">
                    <Spin tip="加载中..." spinning={this.state.loading} size="large" style={{ zIndex: 9999 }}>
                        <Row className="m-input">
                            ...
                            <Col span={5}>
                                {
                                    <Input value={this.state.list.businessTrueName} style={{ width: 260 }} onChange={(e) => this.businessTrueNameChange(e.target.value)} />
                                }
                            </Col>
                        </Row>
                        <Row className="m-input">
                            ...
                            <Col span={5}>
                                {
                                    <Input maxLength={11} value={this.state.list.mobile} style={{ width: 170 }} onChange={(e) => this.mobileChange(e.target.value)} />
                                }
                            </Col>
                        </Row>
                        <Row className="m-input">
                        ...
                        <Row className="m-input">
                            ...
                            <Col span={5}>
                                {
                                    <Input value={this.state.list.dealerAddress} style={{ width: 260 }} onChange={(e) => this.dealerAddressChange(e.target.value)} />
                                }
                            </Col>
                        </Row>
                        <Row className="m-input">
                        ...
                        <Row className="m-input">
                            ...
                            <Col span={5}>
                                <Switch defaultChecked onChange={(e) => this.redCodePowerChange(e)} />
                            </Col>
                        </Row>
                        <Row className="m-input">
                            <Col span={2} style={{ width: 170, textAlign: 'right' }}><span className="u-txt">动画图书馆续费金额</span></Col>
                            <Col span={3}>
                                <Select defaultValue="" style={{ width: 170 }} onChange={(value) => this.renewMoneyChange(value)}>
                            ...
                            {this.state.otherRenewMoney == true ? <Col span={3}>{<InputNumber defaultValue={this.state.oRenewMoney} onChange={(value) => this.oRenewMoneyChange(value)} style={{ width: 170 }} />}</Col> : null}
                        </Row>
                        <Row className="m-input">
                            ...
                            <Col span={5}>
                                <RadioGroup onChange={(e) => this.divideTypeChange(e)} value={this.state.value}>
                                    ...
                                    分成金额
                                    <Select disabled={this.state.dividMoneyFlag} defaultValue="" onChange={(value) => this.dividMoneyChange(value)} style={{ width: 100, marginLeft: 20 }}>
                                        {/* {
                                    ...
                                    分成比例
                                    <Select disabled={this.state.dividePercentFlag} defaultValue="" onChange={(value) => this.dividePercentChange(value)} style={{ width: 100, marginLeft: 20 }}>
                                        {/* {
