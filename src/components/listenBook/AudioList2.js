...
export default class AudioList extends React.Component {
    ...
    //图书列表
    bookFetchFn = async (sleepCode,pageVo) => {
        this.setState({
            bookLoading: true
        });
        var data = await fetch(util.url, {
            ...
            mode: 'cors',
            body: "method=ella.operation.getSleepListenAudioList" + "&content=" + JSON.stringify({sleepCode,pageVo}) + commonData.dataString
        })
            .then(function (res) {
                return res.json();
            });
    ...
    pageChangeFun(pageNum) {
        ...
        this.setState({
            ...
            this.bookFetchFn(this.state.sleepCode, this.state.pageVo);
        });
    }
    ...
    componentDidMount() {
        console.log(this.props.sleepCode);
        this.setState({
            listenCode:this.props.sleepCode
        })
        this.bookFetchFn(this.props.sleepCode,this.state.pageVo);
    }
